...
# ── CRM-added entries ────────────────────────────────────────────────────────
# Questions recorded by voice in the /crm console are written straight INTO the
# dict above (and into agent.AGENT_SYSTEM_PROMPT's Q-list, which must stay in
# sync) — same file, same format, same Q-numbering as every hand-written entry.
# There is no second bank file to keep track of; a CRM entry is just an ordinary
# entry, marked with the comment below so the console knows which ones IT added
# and may therefore delete.
CRM_MARKER = "# added via /crm"
_SOURCE_FILE = os.path.abspath(__file__)
_AGENT_FILE = os.path.join(os.path.dirname(_SOURCE_FILE), "agent.py")
_CRM_ENTRY_RE = re.compile(
    r'^\s*"(Q\d+)":\s*\{\s*' + re.escape(CRM_MARKER) + r'(?:\s+on\s+(\S+))?',
    re.MULTILINE)


def crm_entries() -> dict[str, str]:
    """{qid: date added} for every entry the /crm console wrote into this file."""
    try:
        with open(_SOURCE_FILE, "r", encoding="utf-8") as f:
            src = f.read()
    except Exception as e:
        logger.warning(f"Could not read {_SOURCE_FILE}: {e}")
        return {}
    return {m.group(1): (m.group(2) or "") for m in _CRM_ENTRY_RE.finditer(src)}


CUSTOM_IDS = set(crm_entries())
...
# ── Adding a question at RUNTIME (the /crm console) ─────────────────────────
# The classifier prompt is one big f-string built once at import, with the FAQ
# menu baked into it. Keeping a copy with the menu punched back out lets a new
# entry re-render that same prompt in place — no restart, no duplicated text.
_CLASSIFIER_TEMPLATE = _CLASSIFIER_SYSTEM.replace(_FAQ_MENU, "\x00MENU\x00")


def _rebuild_classifier_menu() -> None:
    """Re-render the classifier's FAQ menu from the current bank."""
    global _FAQ_MENU, _CLASSIFIER_SYSTEM
    _FAQ_MENU = "\n".join(_menu_line(qid, v)
                          for qid, v in CANONICAL_ANSWERS.items())
    _CLASSIFIER_SYSTEM = _CLASSIFIER_TEMPLATE.replace("\x00MENU\x00", _FAQ_MENU)


def next_qid() -> str:
    """The next free Q-number, continuing the hand-written series."""
    n = 0
    for qid in CANONICAL_ANSWERS:
...
def _write(path: str, text: str) -> None:
    """Atomic replace, keeping a .bak of what was there before — these are
    SOURCE files, so a bad write must always be recoverable."""
    try:
...
def _lit(s: str) -> str:
    """A safe Python string literal (json.dumps produces valid Python too)."""
    return json.dumps(s, ensure_ascii=False)


def _source_block(qid: str, q, a: str, comment: str = "") -> str:
    """The entry exactly as it is written by hand in CANONICAL_ANSWERS."""
    head = f'    "{qid}": {{{comment}\n'
    if isinstance(q, dict):
        items = list(q.items())
        body = f'        "q": {{"{items[0][0]}": {_lit(items[0][1])},\n'
        for k, v in items[1:]:
            body += f'              "{k}": {_lit(v)},\n'
        body = body.rstrip(",\n") + "},\n"
    else:
        body = f'        "q": {_lit(q)},\n'
    return head + body + f'        "a": {_lit(a)},\n    }},\n'


def _add_to_router_source(qid: str, q, a: str, when: str) -> None:
    """Insert the entry at the end of CANONICAL_ANSWERS in THIS file."""
    with open(_SOURCE_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    anchor = "CANONICAL_ANSWERS: dict[str, dict] = {"
    start = src.index(anchor)
    end = src.index("\n}\n", start) + 1          # the dict's closing brace
    block = _source_block(qid, q, a, f"  {CRM_MARKER} on {when}")
    _write(_SOURCE_FILE, src[:end] + block + src[end:])


def _add_to_agent_prompt(qid: str, phrasings: list[str], a: str) -> None:
    """Mirror the entry into agent.AGENT_SYSTEM_PROMPT's Q-list.

    The prompt and this bank list the SAME question ids by design (agent.py
    warns at boot if they drift), so an entry added here is added there too, in
    the same shape the hand-written ones use — every phrasing on the Q line
    separated by ' / ' (see Q8), then the answer on the A line."""
    with open(_AGENT_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    anchor = 'AGENT_SYSTEM_PROMPT = """'
    start = src.index(anchor) + len(anchor)
    end = src.index('"""', start)                 # closing delimiter
    q_line = " / ".join(phrasings)
    body = src[start:end]
    # Keep whatever padding sits between the last answer and the closing
    # delimiter, so adding then deleting an entry leaves the file byte-identical.
    tail = body[len(body.rstrip()):]
    _write(_AGENT_FILE,
           src[:start] + body.rstrip() + f"\n{qid}: {q_line}\nA: {a}" + tail
           + src[end:])


def add_entry(question: str, subquestions: list[str], answer: str) -> str:
    """Add ONE approved question+answer to the bank — in memory AND in source.

    `question` is the canonical wording; `subquestions` are alternate phrasings
    (stored as subq2, subq3, ... exactly like a hand-written subq entry, so the
    classifier menu surfaces every one of them). Returns the new qid.

    Two writes, one entry: the block goes into CANONICAL_ANSWERS in this file
    and the matching 'Qn: … / A: …' pair goes into agent.AGENT_SYSTEM_PROMPT, so
    the two never drift. The running process does NOT need a restart — the dict
    and the classifier menu are updated here as well. Its Hinglish wording and
    audio are prepared separately by crm.py (render + TTS cache).
    """
    question = (question or "").strip()
    answer = (answer or "").strip()
    if not question or not answer:
        raise ValueError("both a question and an answer are required")
    subs = [s.strip() for s in (subquestions or []) if s and s.strip()]
    q = question if not subs else dict(
        [("subq1", question)] + [(f"subq{i}", s) for i, s in enumerate(subs, 2)])

    qid = next_qid()
    when = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    # Source first: if a write fails, nothing is half-applied to the live bank.
    _add_to_router_source(qid, q, answer, when)
    _add_to_agent_prompt(qid, [question] + subs, answer)

    CANONICAL_ANSWERS[qid] = {"q": q, "a": answer}
    CUSTOM_IDS.add(qid)
    _rebuild_classifier_menu()
    logger.info(f"FAQ bank += {qid}: '{question[:60]}' ({len(subs)} sub-questions) "
                f"— written into faq_router.py and agent.py")
    return qid


def _replace_in_router_source(qid: str, q, a: str) -> bool:
    """Rewrite one block in place, keeping its position and its comment."""
    with open(_SOURCE_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    pat = re.compile(r'\n    "' + re.escape(qid) + r'": \{(?P<comment>[^\n]*)'
                     r'.*?\n    \},', re.DOTALL)
    m = pat.search(src)
    if not m:
        return False
    block = "\n" + _source_block(qid, q, a, m.group("comment")).rstrip("\n")
    _write(_SOURCE_FILE, src[:m.start()] + block + src[m.end():])
    return True


def _replace_in_agent_prompt(qid: str, phrasings: list[str], a: str) -> bool:
    """Rewrite the matching `Qn: … / A: …` entry in place."""
    with open(_AGENT_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    pat = re.compile(r'\n' + re.escape(qid) + r': .*?(?=\nQ\d+: |\n*""")',
                     re.DOTALL)
    ...
    if not m:
        return False
    block = f"\n{qid}: {' / '.join(phrasings)}\nA: {a}"
    _write(_AGENT_FILE, src[:m.start()] + block + src[m.end():])
    return True


def update_entry(qid: str, question: str, subquestions: list[str],
                 answer: str) -> dict:
    """Edit ANY question in place — memory and both source files.

    Returns {"answer_changed": bool}: when the answer text changes its
    pre-rendered Hinglish wording no longer matches (the hash guard in
    load_variants() would drop it anyway), so the caller re-renders that one
    entry. Raises KeyError for an unknown id, ValueError for empty text."""
    if qid not in CANONICAL_ANSWERS:
        raise KeyError(qid)
    question = (question or "").strip()
    answer = (answer or "").strip()
    ...
    q = question if not subs else dict(
        [("subq1", question)] + [(f"subq{i}", s) for i, s in enumerate(subs, 2)])
    changed = answer != CANONICAL_ANSWERS[qid]["a"]

    if not _replace_in_router_source(qid, q, answer):
        raise RuntimeError(f"{qid} not found in faq_router.py")
    _replace_in_agent_prompt(qid, [question] + subs, answer)

    CANONICAL_ANSWERS[qid] = {"q": q, "a": answer}
    if changed:
        _VARIANTS.pop(qid, None)   # stale wording — do not speak it again
    _rebuild_classifier_menu()
    logger.info(f"FAQ bank ~= {qid}: '{question[:60]}' ({len(subs)} sub-questions"
                f", answer {'changed' if changed else 'unchanged'})")
    return {"answer_changed": changed}


def _drop_from_router_source(qid: str) -> bool:
    """Cut the whole `"Qn": { ... },` block out of CANONICAL_ANSWERS.

    Works for hand-written entries as well as CRM-added ones: every block —
    plain string, subq dict, or implicitly-concatenated answer — ends on its own
    `    },` line, which is what bounds the match."""
    with open(_SOURCE_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    pat = re.compile(r'\n    "' + re.escape(qid) + r'": \{.*?\n    \},',
                     re.DOTALL)
    ...
    if n:
        _write(_SOURCE_FILE, new)
    return bool(n)


def _drop_from_agent_prompt(qid: str) -> bool:
    """Cut the matching `Qn: … / A: …` entry out of AGENT_SYSTEM_PROMPT.

    An answer can run over several lines (Q3, Q23, ...), so the block ends where
    the NEXT question starts — or at the prompt's closing delimiter."""
    with open(_AGENT_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    pat = re.compile(r'\n' + re.escape(qid) + r': .*?(?=\nQ\d+: |\n*""")',
                     re.DOTALL)
    new, n = pat.subn("", src, count=1)
    if n:
        _write(_AGENT_FILE, new)
    return bool(n)


def delete_entry(qid: str) -> dict:
    """Remove ANY question from the bank — memory and both source files.

    Returns {"ok", "custom", "referenced"}. `referenced` lists other places in
    agent.py that still name this id (the ANSWERING_POLICY calls a few entries
    out by number): the entry IS gone, but that prose should be reworded by
    hand, so the console can say so instead of silently leaving a dangling
    reference. Raises KeyError if there is no such entry."""
    if qid not in CANONICAL_ANSWERS:
        raise KeyError(qid)
    was_custom = qid in CUSTOM_IDS
    in_router = _drop_from_router_source(qid)
    in_prompt = _drop_from_agent_prompt(qid)

    CANONICAL_ANSWERS.pop(qid, None)
    CUSTOM_IDS.discard(qid)
    _VARIANTS.pop(qid, None)
    _rebuild_classifier_menu()
    ...
    try:
        with open(_AGENT_FILE, "r", encoding="utf-8") as f:
            for i, line in enumerate(f, 1):
                if re.search(r'\b' + re.escape(qid) + r'\b', line):
                    referenced.append(f"agent.py:{i}: {line.strip()[:90]}")
    except Exception:
        pass
    logger.info(f"FAQ bank -= {qid} (router source: {in_router}, "
                f"prompt: {in_prompt}, stale references: {len(referenced)})")
    return {"ok": in_router or in_prompt, "custom": was_custom,
