...
# Language bias for flux-general-multi. Deepgram REJECTS language_hint on any
# other model — including flux-general-en — with a 400, and a rejected socket
# means the agent never hears a word for the whole call. So the hints are only
# attached to the -multi model, and the supported set is checked up front:
# Flux Multilingual covers exactly these 10 languages.
FLUX_LANGUAGES = {"en", "es", "fr", "de", "hi", "ru", "pt", "ja", "it", "nl"}
FLUX_LANGUAGE_HINTS = [h.strip().lower() for h in
                       os.getenv("FLUX_LANGUAGE_HINTS", "hi,en").split(",")
                       if h.strip()]


def _flux_params() -> str:
    """Flux's turn-taking query string, with every value forced into the range
    the API accepts. Deepgram answers an out-of-range threshold with a 400 and
    closes the socket — which surfaces as an agent that is simply deaf for the
    entire call, so a bad number in .env must never reach the wire."""
    eot = min(max(FLUX_EOT_THRESHOLD, 0.5), 0.9)
    # eager MUST be <= eot (Deepgram rejects the pair otherwise), and cannot go
    # below 0.3. Clamping to eot rather than erroring keeps a mistuned .env on
    # the air — an eager trigger that never fires early just costs latency.
    eager = min(max(FLUX_EAGER_EOT_THRESHOLD, 0.3), 0.9, eot)
    timeout = min(max(FLUX_EOT_TIMEOUT_MS, 500), 60000)
    for name, given, used in (("FLUX_EOT_THRESHOLD", FLUX_EOT_THRESHOLD, eot),
                              ("FLUX_EAGER_EOT_THRESHOLD",
                               FLUX_EAGER_EOT_THRESHOLD, eager),
                              ("FLUX_EOT_TIMEOUT_MS",
                               FLUX_EOT_TIMEOUT_MS, timeout)):
        if given != used:
            logger.warning(f"{name}={given} is outside Deepgram's accepted "
                           f"range — using {used} instead")
    params = (f"&eot_threshold={eot}&eager_eot_threshold={eager}"
              f"&eot_timeout_ms={timeout}")
    if DEEPGRAM_MODEL == "flux-general-multi":
        good = [h for h in FLUX_LANGUAGE_HINTS if h.split("-")[0] in FLUX_LANGUAGES]
        for bad in (h for h in FLUX_LANGUAGE_HINTS if h not in good):
            logger.warning(f"FLUX_LANGUAGE_HINTS: '{bad}' is not one of "
                           f"Flux Multilingual's languages — dropped")
        params += "".join(f"&language_hint={h}" for h in good)
    elif FLUX_LANGUAGE_HINTS:
        logger.info(f"language_hint is only valid on flux-general-multi — "
                    f"not sending hints to {DEEPGRAM_MODEL}")
    return params
...
# BOTH nova-3 (v1) and Flux (v2). Comma-separated here, but sent as a REPEATED
# keyterm= parameter: Deepgram takes a comma-joined value as one literal
# keyterm and silently boosts nothing. Limit is 500 tokens across all terms.
...
class CallSession:
    def __init__(self, ws, caller_id: str = None, call_uuid: str = None):
        ...
        # ── Attention flags (written to the Mongo call document) ─────────────
        # A call is flagged when the caller did NOT get a real answer:
        #   answer_unavailable — the question isn't in the approved bank, so the
        #                        decline / fallback line was spoken instead
        #   no_voice           — a reply WAS generated but not one audio byte
        #                        reached the caller (TTS/cache both failed)
        #   no_response        — the turn produced no reply text at all
        # Every such turn is also appended to self.unresolved with the caller's
        # exact words, so the topic is visible in the DB and the owner can write
        # the proper answer for it right there ("proper_answer" field).
        self._flags = {"answer_unavailable": False,
                       "no_voice": False,
                       "no_response": False}
    ...
    def _flag_issue(self, kind: str, question: str, reply: str = ""):
        """Mark the call as needing attention and record WHICH topic failed.

        The flagged topics are listed in the /unavailable console; the answer for
        one is recorded (by voice) in the /crm console, which adds it to the
        approved question bank the agent answers from."""
        self._flags[kind] = True
        self.unresolved.append({
            # Stable id so answer_unavailable.py can edit/delete exactly this
            # entry, even when a call has several of them.
            "qid": uuid.uuid4().hex,
            "type": kind,
        ...
        logger.warning(f"⚑ {kind} — caller asked: '{(question or '')[:80]}'")

    def _note_turn_outcome(self, question: str, reply: str, completed: bool):
        """Called once per streamed turn, after the reply task settles: decide
        whether the caller actually got a proper, audible answer."""
        if self.faq_state.get("hangup_requested"):
            return  # goodbye turn — the cached closing line is spoken next
        if not reply:
            # The router/LLM produced nothing at all. (A cancelled turn was
            # cut by the caller, not by us, so it isn't a failure.)
            if completed:
                self._flag_issue("no_response", question)
            return
        if completed and not self._reply_audio_sent:
            # Words existed but the line stayed silent — TTS never delivered.
            self._flag_issue("no_voice", question, reply)
        if (self.faq_state.get("last_action") in ("ROUTE", "DECLINE")
                or reply.removesuffix(_INTERRUPT_MARK).strip()
                    in (DECLINE_LINE, ASK_FALLBACK)):
            # Not in the approved bank (or steered away as off-topic): the
    ...
    def _handle_deepgram_message(self, message: str):
        ...
        if msg_type in ("Error", "FatalError"):
            # Deepgram reports a bad request as a message on the socket and
            # then closes it. Without this branch the reconnect loop just
            # retries forever and the ONLY symptom is an agent that never
            # hears anything — with nothing in the log saying why. The usual
            # causes are a rejected query parameter (an out-of-range Flux
            # threshold, language_hint on a non-multi model) or a bad key.
            logger.error(f"Deepgram rejected the stream — "
                         f"{data.get('code', 'no code')}: "
                         f"{data.get('description') or data.get('message') or data}"
                         f" | url={DEEPGRAM_WS_URL}")
            return
        if msg_type in ("Connected", "Metadata", "ConfigureSuccess"):
            logger.debug(f"[Deepgram {msg_type}] {data.get('request_id', '')}")
            return
        if msg_type == "ConfigureFailure":
            logger.warning(f"Deepgram rejected a Configure message: {data}")
            return
        if msg_type == "TurnInfo":
            ...
            # How sure Flux is that the TURN ended — a different question from
            # how sure it is of the words, and only present on EndOfTurn-ish
            # events. Logged (not gated on) because eot_threshold has already
            # applied it server-side; it is what tells you whether a wrongly
            # cut turn was a borderline call or a genuine misfire.
            ...
            if event == "EndOfTurn" and transcript:
                logger.info(f"[Flux EndOfTurn] {transcript}"
                            ...
                            + (f" (eot={eot_conf:.2f})"
                               if isinstance(eot_conf, (int, float)) else ""))
        ...
        if msg_type == "Results":
            ...
            # last_word_end == -1 means the result was ALREADY finalized before
            # the utterance_end_ms gap elapsed — Deepgram documents this as a
            # stale duplicate to ignore. Acting on it was a quiet latency tax:
            # speech_final had just armed the fast TURN_CONFIRM_SECONDS timer,
            # and this branch cancelled it and re-armed the much slower
            # SILENCE_WAIT_SECONDS fallback, delaying every such reply.
            if data.get("last_word_end") == -1:
                logger.debug("[UtteranceEnd] stale (last_word_end=-1) — ignored")
                return
    ...
    async def _deepgram_keepalive(self):
        ...
        try:
            while self.call_active:
                # 3s, not 5s: Deepgram drops an idle socket at 10s (NET-0001)
                # and documents a 3-5s ping. At 5s a single slow event-loop
                # turn while the agent is mid-reply can miss the window and
                # kill STT for the rest of the call; 3s leaves real margin.
    ...
    async def _play_fixed_reply(self, reply: str):
        ...
        interrupted = True
        try:
            sent = await self._play_audio_streaming(
                stream_tts_audio(reply, use_cache=True))
            # SILENT-REPLY FIX: a cache read that yields nothing (corrupt clip,
            # or a first-time synthesis that failed) used to leave the caller
            # listening to silence. Retry ONCE straight from ElevenLabs, cache
            # bypassed, before giving up.
            if sent == 0 and self.streaming_active and not self._retract_turn:
                logger.warning(f"No audio for fixed reply '{reply[:40]}' — "
                               f"retrying synthesis without the cache")
                sent = await self._play_audio_streaming(stream_tts_audio(reply))
            ...
            if retracted:
                ...
                self._mark_last_reply_interrupted()
            if not retracted and not self._reply_audio_sent:
                # Cache miss AND synthesis failed: the caller heard silence
                # where a fixed line (greeting / small talk / FAQ) should have
                # played. Flag it — nobody hears a log line.
                self._flag_issue("no_voice", self._turn_user_text or "", reply)
        ...
        try:
            ...
            async def stream_and_play():
                ...
                try:
                    ...
                    # SILENT-REPLY FIX (live issue: "agent sometimes doesn't
                    # speak"). The WS returned no audio but its feeder ALREADY
                    # consumed the tokens — the old code only recovered when the
                    # stream was still untouched, so this case played nothing at
                    # all and the caller heard dead air. Re-synthesize the text we
                    # already have over HTTP; the words are in full_response, so
                    # nothing is lost.
                    if (sent_bytes == 0 and full_response.strip()
                            and self.streaming_active):
                        spoken = full_response.strip()
                        logger.warning("WS TTS returned no audio for an already-"
                                       "generated reply — re-synthesizing over HTTP")
                        buf = spoken
                        self._spoken_text = spoken
                        while buf and self.streaming_active:
                    ...
                    elif sent_bytes == 0 and not full_response.strip() and self.streaming_active:
                    ...
                    if self._retract_turn:
                        ...
                        # Flag the turn if the caller got no answer, no audio,
                        # or no reply at all (saved to Mongo at cleanup).
                        self._note_turn_outcome(user_text, text, completed)
            ...
            # The turn blew up mid-flight — the caller was left hanging.
            self._flag_issue("no_response", locals().get("user_text", ""))
            self.streaming_active = False
    ...
    async def cleanup(self):
        ...
        if mongo_client is not None:
            conversation_doc = {
                ...
                # ── Attention flags ─────────────────────────────────────────
                # true = something in this call needs a human look.
                "answer_unavailable": self._flags["answer_unavailable"],
        ...
        if self.deepgram_ws:
            try:
                # CloseStream first: it tells Deepgram this is a deliberate
                # end-of-call, so it flushes any buffered audio and closes
                # cleanly instead of logging the call as an aborted stream.
                # Both v1 and v2 accept it; a failure here is not worth caring
                # about, since the close() below tears the socket down anyway.
                try:
